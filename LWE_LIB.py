...
def generatePublicKey(A, B):
    ...
    A = [x % 256 for x in A]
    B = [x % 256 for x in B]
    # Convert vectors A and B to bytes and then to hexadecimal with 64 charactes
    return "00" + bytes(A+B).hex()
...
def generateWalletAddress(public_key):
    version_byte = b'\x00'  # Adding "0x00" as the version byte
...
def printParameters(input_string, A, B, e, secret_vector, secret_vector1, s, q):
    print("\n------Parameters and keys-------")
